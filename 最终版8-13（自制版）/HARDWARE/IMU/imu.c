...
static void NonlinearSO3AHRSupdate(float gx, float gy, float gz, float ax, float ay, float az, float mx, float my, float mz, float twoKp, float twoKi, float dt)
{
    ...
    dq0 = (-q1 * gx - q2 * gy - q3 * gz);
    dq1 = (q0 * gx + q2 * gz - q3 * gy);
    dq2 = (q0 * gy - q1 * gz + q3 * gx);
    dq3 = (q0 * gz + q1 * gy - q2 * gx);
...
#define Params_Kp 1.0f
#define Params_Ki  0.05f
...
void IMUSO3Thread(void)
{
    ...
    NonlinearSO3AHRSupdate(MPU6050_Raw_Data.Gyro_Rad_X, MPU6050_Raw_Data.Gyro_Rad_Y, MPU6050_Raw_Data.Gyro_Rad_Z,
                           MPU6050_Raw_Data.Accel_X, MPU6050_Raw_Data.Accel_Y,MPU6050_Raw_Data.Accel_Z,
                           0, 0, 0,
                           Params_Kp,Params_Ki,0.5f*dt);
